...
def run(args):
    ...
    for i in range(N_ITERATIONS):
        ...
        print(f"Learned GP lengthscale = {gp_model.covar_module.base_kernel.base_latent_kernel.lengthscale.item()}")
        print(f"Learned GP lengthscale (instruction) = {gp_model.covar_module.base_kernel.instruction_kernel.lengthscale.item()}")
    ...
    if args.track_ground_truth:
        ...
        Y_unique = Y[indices]
        ground_truth_path = f"{OUT_DIR}/ground_truth_seed-{args.seed}.pt"
        torch.save((X_unique, Y_unique), ground_truth_path)
        print(f"Saved {len(X_unique)} unique ground truth (x,y) pairs to {ground_truth_path}")
