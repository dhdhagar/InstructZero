...
class Llama3_8b(LLM):
    ...
    def __init__(self, config, needs_confirmation=False, disable_tqdm=True):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            "google/gemma-1.1-2b-it", token=hf_token
        )
    ...
    def generate_text(self, prompts, n):
        ...
        for prompt in tqdm(prompts):
            ...
            generate_ids = self.model.generate(**tokenized, max_new_tokens=32)
            text.append(
...
class Flan_T5(LLM):
    ...
    def __init__(self, config, needs_confirmation=False, disable_tqdm=True):
        ...
        self.model = AutoModelForSeq2SeqLM.from_pretrained(
            "google/flan-t5-xxl", torch_dtype=torch.float16
...
class GPT_Forward(LLM):
    ...
    def __async_generate(self, prompt, n):
        ml = [
            [{"role": "user", "content": p.replace("[APE]", "").strip()}]
            for p in prompt
        ]
        answer = None
