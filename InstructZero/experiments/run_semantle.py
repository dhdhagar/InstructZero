...
that are similar to it in meaning.\n\nWord: """
    ...
    def eval(self, soft_prompts=None, no_prompt=False):
        add_decoding_kwargs = {}
        self.soft_prompts.append(soft_prompts.to('cpu') if type(soft_prompts) is torch.Tensor else soft_prompts)
        if not no_prompt:
...
def get_textual_warmstart(args):
    ...
    cands = [(cand[0], float(cand[1])) for cand in cands]
...
def run(args):
    ...
    print(f"Best textual warmstart point: {sorted(warmstart, key=lambda x: -x[1])[0]}\n")
