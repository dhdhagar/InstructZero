...
class CombinedStringKernel(Kernel):
    ...
    def forward(self, z1, z2, **params):
        ...
        latent_space_kernel_inv = torch.inverse(
            latent_space_kernel + 0.0001 * torch.eye(len(self.latent_train)).to(latent_space_kernel.device))

        kernel_val = K_z1_training @ latent_space_kernel_inv @ (
            K_train_instruction) @ latent_space_kernel_inv @ K_z2_training.T
        if check_dim > 0:
